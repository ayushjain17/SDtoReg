...
class StateDiagramWidget(Frame):
    def __init__(self, master, width, height):
        ...
        self.canvas.bind("<ButtonPress-1>", self.canvasDrag_start)
        self.canvas.bind("<B1-Motion>", self.canvasDrag_move)
        # self.canvas.create_rectangle(0, 0, width-1, height-1)
        self.canvas.pack()

        self.add_button = Button(self, text="Add State", command=self.add_node, cursor='hand2')
        self.add_button.place(x=10, y=10)
        self.check_button = Button(self, text="Check", command=self.print_all, cursor='hand2')
        self.check_button.place(x=90, y=10)
        self.clear_button = Button(self, text="Clear", command=self.clear, cursor='hand2')
        self.clear_button.place(x=150, y=10)

        self.define_button = Button(self, text="Define Automata", command=self.define, cursor='hand2')
        self.define_button.place(x=width-115, y=height-40)
    ...
    def canvasDrag_start(self, event):
        widget = event.widget
        canvasx = widget.canvasx(event.x)
        canvasy = widget.canvasy(event.y)
        item_tags = widget.itemcget(widget.find_closest(canvasx, canvasy)[0], "tags").split(' ')
    ...
    def canvasDrag_move(self, event):
    ...
    def clear(self):
        ...
        self.canvas.create_rectangle(0, 0, self.width-1, self.height-1)
        self.objs = []
        self.nodes = {}
        self.arrows = []
        self.loops = []
    ...
    def define(self):
        ...
        for q in self.nodes.values():
            ...
            if q.isFinal:
                F.append(q.label)
        if not F:
            self.machine.insert(INSERT, "No final state defined.")
            return
        Sigma = []
        d = []
...
if __name__ == '__main__':
    ...
    StateDiagramWidget(root, width=root.winfo_screenwidth() * 2 / 3, height=root.winfo_screenheight() * 1 / 2).pack()
    root.mainloop()
