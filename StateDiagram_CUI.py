class Equation:
    ...
    def concat(self, var, st):
        # orig = self.eq[var]
        ad_st = f"{st}*" if len(st) == 1 else f"({st})*"
        orig = '' if (self.eq[var] == '$' or self.eq[var] == '') else self.eq[var] if '+' not in self.eq[var] else f"({self.eq[var]})"
        # if orig == '$' or orig == '':
        #     orig = ''
        # else:
        #     orig = self.eq[var] if '+' not in self.eq[var] else f"({self.eq[var]})"
        self.eq[var] = f"{orig}{ad_st}"
